...
- The local model baseline is Qwen2.5-VL 3B with first-use automatic download, with Qwen2.5-VL 7B available as a higher-quality option. Remaining detail needed: packaged smoke evidence, local runtime contract evidence, and sanitized acceptance records showing the selected runtime satisfies the Responses API-compatible Agent contract without fragile prompt-only tool emulation.
