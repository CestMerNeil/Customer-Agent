...
- Preserve and productize the Electron app's local-model advantage: the app must run with a managed local Responses API-compatible runtime and model provisioning path that does not require users to manually install third-party LLM tools. If the current local runtime candidate cannot satisfy the required native tool-call and tool-result behavior, the runtime candidate must be replaced rather than hidden behind prompt-only JSON emulation.
...
- `local-model-runtime-provisioning`: Local Responses API-compatible runtime preservation, bundled or automatically downloaded runtime binaries, managed model downloads, health checks, runtime-candidate rejection criteria, and platform release gates.
...
- Requires real Responses API-compatible chat/tool, embedding, and local multimodal model endpoints for Agent and product-knowledge extraction acceptance. The default path is first-launch or first-use automatic download of a small approved local model profile. Product-image multimodal extraction must use an approved Qwen2.5-VL local multimodal profile; remote multimodal fallback is out of scope. The exact model ID, license, file size, checksum, runtime compatibility, and Responses contract support are finalized during model-profile implementation.
