...
- **Incremental, page-by-page migration behind the new shell.** Land the token foundation and layout shell first, then migrate pages so the app stays runnable and `verify:flow` stays green throughout. Alternative considered: big-bang rewrite; rejected because it breaks the verifiable loop the project depends on.
