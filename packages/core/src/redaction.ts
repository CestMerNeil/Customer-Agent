...
const redactionPatterns: Array<{ category: RedactionIssue["category"]; name: string; pattern: RegExp; replacement: string }> = [
  ...
  { category: "secret", name: "token-field", pattern: /\b([A-Za-z0-9_ -]*token|access_token)\s*[:=]\s*(?:\$\{\{[^}]+\}\}|\$[A-Za-z_][A-Za-z0-9_]*|[^,\s;]+)/gi, replacement: "$1=[REDACTED]" },
  { category: "secret", name: "api-key-field", pattern: /\bapi[-_ ]?key\s*[:=]\s*(?:\$\{\{[^}]+\}\}|\$[A-Za-z_][A-Za-z0-9_]*|[^,\s;]+)|\bauthorization\s*:\s*bearer\s+(?:\$\{\{[^}]+\}\}|\$[A-Za-z_][A-Za-z0-9_]*|[^,\s;]+)/gi, replacement: "[REDACTED]" },
...
export function scanSensitiveText(value: string): RedactionScanResult {
  ...
  for (const item of redactionPatterns) {
    const hasSecret = Array.from(value.matchAll(item.pattern)).some((match) => !isReferenceOnly(match[0]));
    if (hasSecret) {
      issues.push({ category: item.category, pattern: item.name });
    }
    item.pattern.lastIndex = 0;
    redacted = redacted.replace(item.pattern, (match) => isReferenceOnly(match) ? match : item.replacement);
  }
